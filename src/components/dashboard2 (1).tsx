...
export default function FinancialDashboard() {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>([
    ...
    { id: 3, type: 'income', amount: 500, description: "Freelance", date: "2023-06-03", goalId: 1 },
  ])
    ...
    if (newTransaction.amount > 0 && newTransaction.description) {
      if (newTransaction.type === 'income') {
        ...
        const updatedGoals = goals.map(goal => ({
          ...goal,
          current: goal.current - newTransaction.amount
        }))
  ...
  const addIncomeToGoal = () => {
    if (selectedGoalId !== null) {
      const updatedGoals = goals.map(goal => 
        goal.id === selectedGoalId 
          ? { ...goal, current: goal.current + newTransaction.amount }
          : goal
      )
      setGoals(updatedGoals)
      setTransactions([...transactions, { ...newTransaction, id: transactions.length + 1, date: new Date().toISOString().split('T')[0], goalId: selectedGoalId }])
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4">
      ...
      <Dialog open={isIncomeDialogOpen} onOpenChange={setIsIncomeDialogOpen}>
        <DialogContent className="bg-gray-800 text-white">
          <DialogHeader>
            <DialogTitle>Escolha uma meta para investir</DialogTitle>
          </DialogHeader>
          <Select onValueChange={(value) => setSelectedGoalId(Number(value))}>
          ...
          <Button onClick={addIncomeToGoal}>Confirmar</Button>
        </DialogContent>
      </Dialog>
    </div>
  )
